...
MZoneActivityState::MZoneActivityState(const MZoneActivityState &state)
{
	...
	for(int i=0; i<ap->numPopHistBinsPC; i++)
	{
		histPCPopAct[i]=state.histPCPopAct[i];
	}
...
void MZoneActivityState::writeState(fstream &outfile)
{
	stateRW(false, (fstream &)outfile);
}

bool MZoneActivityState::equivalent(const MZoneActivityState &compState)
{
	bool eq;
	...
	for(int i=0; i<cp->numBC; i++)
	{
		eq=eq && (threshBC[i]==compState.threshBC[i]);
		eq=eq && (vBC[i]==compState.vBC[i]);
	}
	for(int i=0; i<cp->numPC; i++)
	{
		eq=eq && (threshPC[i]==compState.threshPC[i]);
		eq=eq && (vPC[i]==compState.vPC[i]);
	}
	for(int i=0; i<cp->numNC; i++)
	{
		eq=eq && (vNC[i]==compState.vNC[i]);
		eq=eq && (synIOPReleaseNC[i]==compState.synIOPReleaseNC[i]);
	}

	return eq;
}

vector<float> MZoneActivityState::getGRPCSynWeightLinear()
{
	vector<float> retVec;
...
//vector<vector<float> > MZoneActivityState::getGRPCSynWeight()
//{
...
//		vector<float> pcVec;
//
//		pcVec.resize(cp->numpPCfromGRtoPC);
//
//		for(int j=0; j<cp->numpPCfromGRtoPC; j++)
//		{
//			pcVec[j]=pfSynWeightPC[j];
//		}
//
//		retVec.push_back(pcVec);
//	}
//
//	return retVec;
//}
