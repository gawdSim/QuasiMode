...
CBMState::CBMState(fstream &actPFile, fstream &conPFile, unsigned int nZones, int goRecipParam)
{
	...
	newState(actPFile, conPFile, nZones, innetCRSeed, mzoneCRSeed, mzoneARSeed, goRecipParam);
...
void CBMState::newState(fstream &actPFile, fstream &conPFile, unsigned int nZones,
			int innetCRSeed, int *mzoneCRSeed, int *mzoneARSeed, int goRecipParam)
{
	...
	innetConState=new InNetConnectivityState(conParams, actParams->msPerTimeStep, innetCRSeed, goRecipParam);
