...
{
public:
	InNetConnectivityState(ConnectivityParams *parameters, unsigned int msPerStep, int randSeed, int goRecipParam);
...
protected:
	...
	virtual void connectGOGOBias(CRandomSFMT *randGen);
		...
		virtual void connectGOGODecayP(CRandomSFMT *randGen, int goRecipParam);
